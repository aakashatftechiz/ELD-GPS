...
package com.example.eldgps.sync.viewmodels
...
import com.example.eldgps.sync.repository.EmulatorRepository
import com.example.eldgps.sync.repository.SSERepository
import com.example.eldgps.sync.repository.StopPointsRepository
import com.example.eldgps.sync.repository.SyncRepository
import com.example.eldgps.sync.repository.TripPointsRepository
...
//            // Show a Toast error message and hide the progress bar.
     ...
     */
    class Factory(val app: Application, private val sharedPreferences: SharedPreferences) :
    ...
    var sseEvents = MutableLiveData<SSEEventData>()
        private set
    var syncEvents = MutableLiveData<String>()
        private set
    fun getSyncEvents() = viewModelScope.launch {
        repository.syncEventsFlow.onEach { sseEventData ->
                syncEvents.postValue(sseEventData)
            }.catch {
                syncEvents.postValue(syncEvents.value)
            }.launchIn(viewModelScope)
    }

    fun getSSEEvents() = viewModelScope.launch {
        repository.sseEventsFlow.onEach { sseEventData ->
